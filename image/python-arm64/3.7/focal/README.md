...
```console
$ docker run -it --rm --name=dev-consul -e CONSUL_BIND_INTERFACE=eth0 taylorific/consul-arm64
```
...
```console
$ docker run -it --rm -e CONSUL_BIND_INTERFACE=eth0 taylorific/consul-arm64 agent -dev -join=172.17.0.2
... server 2 starts
docker run -it --rm -e CONSUL_BIND_INTERFACE=eth0 taylorific/consul-arm64 agent -dev -join=172.17.0.3
... server 3 starts
```
